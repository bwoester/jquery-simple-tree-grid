 ...
 *  - work based on http://code.google.com/p/etherpad/source/browse/trunk/infrastructure/ace/www/skiplist.js rev 81287c8af1ac
 *  - wrap skip list source in self executing anonymous function
 *  - register factory method "newSkipList" in namespace "bwoester"
 *  - use "window.console" instead of "console"
 *  - add some annotations to silence google closure warnings
 *
...
bwoester.newSkipList = function() {
  ...
  /**
   * @param {boolean=} byWidth (optional).
   */
...
*/
  var self = {
    ...
    atIndex: function(i) {
      if (i < 0) window.console.warn("atIndex("+i+")");
      if (i >= numNodes) window.console.warn("atIndex("+i+">="+numNodes+")");
    ...
    splice: function(start, deleteCount, newEntryArray) {
      if (start < 0) window.console.warn("splice("+start+", ...)");
      if (start + deleteCount > numNodes) {
	window.console.warn("splice("+start+", "+deleteCount+", ...), N="+numNodes);
	window.console.warn("%s %s %s", typeof start, typeof deleteCount, typeof numNodes);
